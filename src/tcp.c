...
static int tcp_fsm(struct tcp_conn_tcb *conn, struct tcp_hdr *rs, struct ip_hdr *ip_hdr, size_t bsize)
{
    switch (conn->state) {
    case TCP_CLOSED:
    ...
    case TCP_LISTEN:
        ...
        if (rs->tcp_flags & TCP_SYN) {
            ...
            sockaddr.port = rs->tcp_dport;
            sock = find_tcp_socket(&sockaddr);
            if (!sock) {
    ...
    case TCP_ESTABLISHED:
        ...
        if ((rs->tcp_flags & TCP_ACK) && (rs->tcp_flags & TCP_PSH) &&
            ...
            struct nstack_sockaddr srcaddr = {
                .inet4_addr = ip_hdr->ip_src, .port = rs->tcp_sport,
            };
